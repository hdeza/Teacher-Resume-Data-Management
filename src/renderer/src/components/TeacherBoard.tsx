...
export default function TeacherBoard() {
  ...
  useEffect(() => {
    let teachersInfo: teacherCardInfo[] = []
    ...
    async function fetchTelefono(dni: string, newTeacher: teacherCardInfo) {
      let { data: telefonos, error } = await supabase
        .from('telefonos')
        .select('*')
        .eq('docente', dni)
      if (error) {
    ...
    async function fetchTeacher() {
      ...
      if (error) {
        ...
        docentes.forEach((teacher) => {
          ...
          fetchEntrevista(teacher.documentoid, newTeacher)
          fetchCiudad(teacher.ciudad, newTeacher)
          fetchTelefono(teacher.documentoid, newTeacher)
          fetchEstudio(teacher.documentoid, newTeacher)
          fetchTrabajo(teacher.documentoid, newTeacher)
          fetchResume(teacher.documentoid, newTeacher)

          teachersInfo.push(newTeacher)
        })
      }
    }

    fetchTeacher()
    setTeacherData(teachersInfo)
    //finalmente mostramos en la consola los datos formateados desde la base de datos
